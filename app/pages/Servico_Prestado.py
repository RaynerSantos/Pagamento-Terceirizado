...
import gspread
from utils import client, credentials, PROJETOS, PERIODOS
...
if PERIODOS[0] in periodo_usuario.values:
    ...
    st.write(f"Valor a receber no período de *{PERIODOS[0]}*:  **R$0,00**")
...
with st.form(key="servico"):
    ...
    PERIODO = st.selectbox(label="Informe o período no qual o projeto ocorreu", options=[PERIODOS])
